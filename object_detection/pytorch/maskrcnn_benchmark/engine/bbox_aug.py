...
def im_detect_bbox_aug(model, images, device, use_mkldnn=False):
    ...
    boxlists_i = im_detect_bbox(
        model, images, cfg.INPUT.MIN_SIZE_TEST, cfg.INPUT.MAX_SIZE_TEST, device, use_mkldnn
    )
    ...
    for scale in cfg.TEST.BBOX_AUG.SCALES:
        ...
        boxlists_scl = im_detect_bbox_scale(
            model, images, scale, max_size, device, use_mkldnn
        )
        add_preds_t(boxlists_scl)

        if cfg.TEST.BBOX_AUG.SCALE_H_FLIP:
            boxlists_scl_hf = im_detect_bbox_scale(
                model, images, scale, max_size, device, hflip=True, use_mkldnn=use_mkldnn
            )
...
def im_detect_bbox(model, images, target_scale, target_max_size, device, use_mkldnn=False):
    """
    ...
    return model(images.to_mkldnn()) if use_mkldnn else model(images.to(device))


def im_detect_bbox_hflip(model, images, target_scale, target_max_size, device, use_mkldnn=False):
    """
    ...
    images = [transform(image) for image in images]
    images = to_image_list(images, cfg.DATALOADER.SIZE_DIVISIBILITY)
    boxlists = model(images.to_mkldnn()) if use_mkldnn else model(images.to(device))
...
def im_detect_bbox_scale(model, images, target_scale, target_max_size, device, hflip=False, use_mkldnn=False):
    """
    Computes bbox detections at the given scale.
    Returns predictions in the scaled image space.
    """
    if hflip:
        boxlists_scl = im_detect_bbox_hflip(model, images, target_scale, target_max_size, device, use_mkldnn)
    else:
        boxlists_scl = im_detect_bbox(model, images, target_scale, target_max_size, device, use_mkldnn)
    return boxlists_scl
